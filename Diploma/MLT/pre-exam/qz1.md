...
X = np.array([[0, 0], [1, 2], [3, 6], [5, 10]], dtype=float)

# Center the data
X_centered = X - np.mean(X, axis=0)

# Covariance matrix (unbiased, ddof=1)
cov_unbiased = np.cov(X_centered, rowvar=False)
eigenvalues_unbiased, _ = np.linalg.eigh(cov_unbiased)

# Covariance matrix (biased, ddof=0)
cov_biased = np.cov(X_centered, rowvar=False, ddof=0)
eigenvalues_biased, _ = np.linalg.eigh(cov_biased)

print(f"Unbiased eigenvalues: {eigenvalues_unbiased}")
print(f"Biased eigenvalues: {eigenvalues_biased}")

# Using sklearn PCA
pca = PCA()
pca.fit(X)
print(f"sklearn explained variance (unbiased): {pca.explained_variance_}")
...
> **Option A (Biased Covariance Matrix / Divided by $N=4$):** If your course defines standard PCA using $\frac{1}{N} \sum d_i^2$:
>
> $$
...
> **Option B (Unbiased Covariance Matrix / Divided by $N-1=3$):** If your course follows standard software packages like `scikit-learn` that use sample variance $\frac{1}{N-1} \sum d_i^2$:
>
> $$
